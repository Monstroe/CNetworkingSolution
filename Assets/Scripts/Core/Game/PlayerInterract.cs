...
public class InteractableInteract : MonoBehaviour
{
    ...
    {
        grabCached = true;
        Player.Instance.SendToServerObject(PacketBuilder.PlayerGrabRequest(interactable.Id), TransportMethod.Reliable);
    ...
    {
        grabCached = true;
        Player.Instance.SendToServerObject(PacketBuilder.PlayerDropRequest(), TransportMethod.Reliable);
    ...
    void Interact()
    {
        ...
        {
            Player.Instance.SendToServerObject(PacketBuilder.PlayerInteractRequest(), TransportMethod.Reliable);
        }

        // DEBUG SPAWN ITEMS
        if (Input.GetKeyDown(KeyCode.X))
        {
            ClientManager.Instance.CurrentLobby.SendToServer(PacketBuilder.ObjectSpawnRequest("Assets/GameAssets/Objects/Client/Items/ClientBasicItem.prefab", transform.position, transform.rotation, false), TransportMethod.Reliable);
        }
    ...
    void Animate()
    {
        ...
        {
            Player.Instance.SendToServerObject(PacketBuilder.PlayerAnim(Player.Instance.IsWalking, Player.Instance.IsSprinting, Player.Instance.IsCrouching, Player.Instance.IsGrounded, Player.Instance.Jumped, Player.Instance.Grabbed), TransportMethod.Reliable);
        }
    }
