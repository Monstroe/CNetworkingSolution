...
public class LobbyInitializer : MonoBehaviour
{
    ...
    public void GoToGameScene()
    {
        FadeScreen.Instance.Display(true, fadeDuration, () =>
        {
            if (ServerManager.Instance != null)
            {
                ...
                SceneManager.LoadSceneAsync(GameResources.Instance.GameSceneName, LoadSceneMode.Additive).completed += (asyncOperation) =>
                {
                    SceneManager.SetActiveScene(SceneManager.GetSceneByName(GameResources.Instance.GameSceneName));
                };
            }
            else
            {
    ...
    /*private void LobbyCreateRequested()
    {
