...
public class RpcBus
{
    ...
    private readonly static Dictionary<Type, ulong> serviceCache = new Dictionary<Type, ulong>();

    public static ulong GetServiceId(Type type)
    {
        if (serviceCache.TryGetValue(type, out ulong id))
        {
            return id;
        }

        var attr = type.GetCustomAttribute<ServiceIdAttribute>() ?? throw new Exception($"ServiceIdAttribute missing on {type.FullName}");
        id = NetResources.GenerateHashKey(attr.ServiceId);
        serviceCache[type] = id;
        return id;
    }

    public static ulong GetServiceId<T>()
    {
        return GetServiceId(typeof(T));
    }

    public void RegisterService(INetService instance)
    {
