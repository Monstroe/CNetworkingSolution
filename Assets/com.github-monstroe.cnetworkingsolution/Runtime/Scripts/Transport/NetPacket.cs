...
public class NetPacket
{
    ...
    public T? ReadNullableEnum<T>(bool moveIndexPosition = true) where T : struct, Enum
    {
        bool hasValue = ReadBool();
        if (!hasValue)
        {
    ...
    public T[] ReadEnums<T>(bool moveIndexPosition = true) where T : Enum
    {
        int length = ReadInt();
        int typeSize = length * sizeof(ushort) + sizeof(int);
    ...
    public T?[] ReadNullableEnums<T>(bool moveIndexPosition = true) where T : struct, Enum
    {
        int cachedIdx = CurrentIndex;
        int length = ReadInt();
        var value = new T?[length];
    ...
    public object ReadObject(Type parameterType, bool moveIndexPosition = true)
    {
        ...
        try
        {
            ...
            if (parameterType.IsEnum)
            {
                return Enum.ToObject(parameterType, ReadUShort());
            }
